...
function getNodePositions() {
  ...
  // Measure actual header height so nodes never get clipped behind it
...
canvas.addEventListener('mousemove', e => {
  ...
  for (const rel of RELATIONSHIPS) {
    ...
    const dist = pointToSegmentDist(e.clientX, e.clientY, a.x, a.y, b.x, b.y);
    if (dist < 10) {
      const fromChar = CHARACTERS.find(c => c.id === rel.from);
      const toChar = CHARACTERS.find(c => c.id === rel.to);
      found = `${fromChar?.name} ↔ ${toChar?.name}\n${rel.label}`;
      break;
    }
  }
